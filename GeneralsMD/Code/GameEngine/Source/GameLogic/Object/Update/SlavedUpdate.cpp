...
UpdateSleepTime SlavedUpdate::update()
{
	...
	{
		if (m_framesToWait > 0 && m_framesToWait > now)
		{
			return UPDATE_SLEEP(m_framesToWait - now);
		}
		m_framesToWait = now + SLAVED_UPDATE_RATE;
	}
	...
	if (healthPercentage <= data->m_repairWhenHealthBelowPercentage)
	{
		...
		doRepairLogic();
		return m_framesToWait > now ? UPDATE_SLEEP(m_framesToWait - now) : UPDATE_SLEEP_NONE;
	}
...
//-------------------------------------------------------------------------------------------------
UpdateSleepTime SlavedUpdate::calcSleepTime() const
{
#ifdef SLAVE_SLEEPY
				...
				if( !data->m_weldingSysName.isEmpty() )
				{
					...
					if( tmp )
					{
						...
						if( weldingSys )
						{
							...
							Real time = (Real)(weldFrames * static_cast<float>(LOGICFRAMES_PER_SECOND));
...
//-------------------------------------------------------------------------------------------------
void SlavedUpdate::informMySlaverSelfInfo()
{
	if( m_noAggregateHealth )
		return;
